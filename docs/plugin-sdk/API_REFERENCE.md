...
מחיקת תיקייה על כל תוכנה, בתוך תיקייה מאושרת. בשונה מ-`fs.deleteFile`,
שמוגבל לקבצים בלבד. הפעולה idempotent — תיקייה שאינה קיימת מסתיימת בהצלחה.
...
חייבים להיות בתוך תיקייה מאושרת. תיקיית האב של `to` נוצרת אם אינה קיימת.
...
שגיאות אפשריות: `error.forbidden` (נתיב מחוץ לתיקייה מאושרת),
`error.invalid_params` (פרמטר חסר, או שהיעד כבר קיים),
