...
- הקובץ נטען כסקריפט רגיל ולא כמודול — `import`/`export` אינם נתמכים. מי שמשתמש במודולים צריך לארוז (bundle) לקובץ אחד.
...
* **יש לקרוא ישירות ממחוות משתמש מהימנה.** הפעילו `queryLocalFonts()` מתוך handler של `click` או `keydown` מהימנים. ללא transient user activation מתקבל `SecurityError` עוד לפני שער ההרשאה של אוצריא, ולכן הכשל אינו נרשם כדחיית הרשאה.
* **אין `fonts.queryLocal` ב-`Otzaria.call`.** כמו קריאת הלוח, זו הרשאת דפדפן ולא מסלול RPC: היא נאכפת ב-`onPermissionRequest`, ומה שהתוסף קורא לו הוא ה-API הרגיל של הדפדפן. דחיית ההרשאה עצמה נרשמת בלוג הריצה.
