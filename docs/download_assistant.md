...
1. **כל בית שירד עובר hash פעם אחת בדיוק.** macOS ו-Linux: בזרימה, תוך כדי
   הכתיבה לדיסק — אין מעבר שני. Inno: ה-hash של עמוד ההורדה (שמקבל את ה-sha
   תמיד) הוא היחיד; `PromoteToCache` בודק גודל בלבד, ובדיקת מטמון והרכבה נשענות
   על החותם.
2. **הקובץ המורכב אינו עובר hash משלו.** כל חלק אומת מול ה-sha256 שלו, השרשור
   דטרמיניסטי, והחלקים וה-hash השלם נכתבו מאותו קובץ באותה ריצה של
   `split_release_asset.sh`. מה שנבדק: שכל חלק נכתב במלואו (מספר הבתים שנוסף
   שווה לגודל החלק) ושהגודל הסופי שווה ל-`size` של הנכס. hash שלם היה מוסיף
   17-35 שניות ולא היה תופס דבר שהבדיקות האלה אינן תופסות, מלבד באג בקוד
   השרשור עצמו — וזה נבדק בבדיקות היחידה של כל מימוש. ה-sha256 השלם נשאר
   במניפסט עבור `assemble_split_asset.sh`/`.ps1`.
...
**נמדד** (`/LOG`, אותו מחשב, מתקין FULL של 0.9.97 — ‎2,012,390,081‎ בתים):
...
לכל הריצה. הרכבה של אותו קובץ משני חלקים (1.2GB + 0.75GB): 10.7 ש׳ בלי hash
לקובץ השלם, והתוצאה זהה בית-בבית (sha256 חיצוני); הפעלה חוזרת מזהה אותה לפי
החותם בלי hash.
...
* **בלי hash לקובץ המורכב** (הנימוק בסעיף 2 של "מהירות — אחרי שהבתים הגיעו"):
  `AppendFileTo` מעתיק בפרוסות של 64MB, מוודא שכל פרוסה הועתקה במלואה ושמיקום
  היעד התקדם בדיוק בגודל החלק, ומדווח התקדמות בבתים ("מחבר את הקבצים"). הקובץ
  מקבל את שמו הסופי אחרי בדיקת הגודל הכולל, ואז נכתב לו חותם במטמון
...
* **הרכבה**: ישר לתיקיית היעד, לקובץ עבודה באותה צורת שם. בהפעלה חוזרת גודל קובץ
  ...
  (הקובץ המורכב אינו עובר hash שלם).
...
* **מגבלת ה-API** (403/429, נפוצה ברשת מסוננת שבה משתמשים רבים חולקים IP): אם
...
`mv app app.otzaria-old && mv prepared app`, שחזור כשהשני נכשל, ומחיקת הישן
ותיקיית העבודה בהצלחה. אוצריא שלא יצאה תוך 2 דקות — הסקריפט כותב
...
ל-bundle יחד עם החותם **לפני** החתימה, ונחתם לפניה; המניפסט נסרק מה-bundle
החתום. ב-Linux הוא וחותם השחרור נכנסים ל-`app/` של חבילת ה-FULL (x64,
...
נותר: בין שני שינויי השם יש רגע שבו ההתקנה אינה בנתיב. תהליך שנהרג בדיוק שם
משאיר אותה ב-`<שם>.otzaria-old`, בלי שחזור אוטומטי.
