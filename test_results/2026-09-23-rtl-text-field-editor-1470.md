...
**בדיקות מקוריות:** `fix/rtl-text-field-editor-1470` על `upstream/dev` 780ae3f94 · **פלטפורמה:** macOS 27.0 · **Flutter:** 3.47.2 · **תאריך:** 23.9.2026

## הבאג
`AGENTS.md` קובע להשתמש ב־`RtlTextField` לשדות קלט. בכל `lib/`
...
## סוויטה מלאה מול הבסיס המקורי, לפני rebase ותיקון כיוון הספרות
...
## בדיקת תיקון כיוון הספרות אחרי rebase

בדיקות widget חדשות מפעילות חיצים בעברית, באנגלית, בתגים, במספרים, בשעות,
במספר עם פסיק ובטקסט ארוך; הן כוללות גם Shift וניווט ברמת מילה. בדיקות
`scrollController` מאמתות חיבור וגלילה בפועל. לפני התיקון נכשלו שישה תרחישי
מספרים ובחירה; אחרי התיקון עברו כולן.

`flutter analyze` — ללא בעיות. 129 בדיקות ממוקדות עברו.
בהרצת הסוויטה המלאה עברו 15,849 בדיקות ו־27 נכשלו. הרצה של עשרת קובצי
הבדיקות שנכשלו על `dev` נקי הניבה **בדיוק את אותם 27 כשלים**, ללא כשל ייחודי
לענף. הכשלים כוללים תלות במנוע החיפוש הנייטיבי וב־PDFium שאינם זמינים
בסביבת ההרצה המקומית.
